...
import { getIdentifiersInProgram, getLambdaExpression } from '../utils/uniqueIds'
import WebGPUTransformer from './transformer'
...
export function transpileToWebGPU(program: es.Program) {
  ...
  const lambdaFn = getLambdaExpression(program)
  console.log('identifiers:', identifiers)
  console.log('lambdaFn:', lambdaFn)

  const transformer = new WebGPUTransformer(program, create.identifier('__initDevice'))
  // probably use this to check whether the lambda function is ok for use.
